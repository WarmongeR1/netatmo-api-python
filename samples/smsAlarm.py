...
if someLost and 'external' in someLost: message.append(
    "Sensor is no longer active")

# Condition 3 : The outdoor module battery is dying
volts = devList.lastData()['external'][
    'battery_vp']  # I suspect that this is the total Voltage in mV
if volts < 5000: message.append(
    "External module battery needs replacement")  # I will adjust the threshold over time
...
if message:
    if not os.path.exists(MARKER_FILE):
        ...
        open(MARKER_FILE,
             "w").close()  # Just to create the empty marker file and avoid to resend the same alert
else:
    if os.path.exists(MARKER_FILE):
        os.remove(MARKER_FILE)
        for p in ('<phone1>', '<phone2>'):
            lsms.sendSMS(p,
                         "END of WEATHER alert, current temperature is %s°C" % curT)
